...
import gk.tweetsched.api.service.TweetService;
...
public class ApiVerticle extends AbstractVerticle {
    ...
    private void configureRoutes(Router router) {
        router.route("/").handler(routingContext -> {routingContext.reroute("/assets/index.html");});
    ...
    private void deleteOne(RoutingContext routingContext) {
        ...
        try {
            tweetService.delete(Integer.valueOf(id));
            routingContext.response().setStatusCode(NO_CONTENT).end();
        } catch (NumberFormatException e) {
            routingContext.response().setStatusCode(BAD_REQUEST).end();
        }
    }
    ...
    private void getOne(RoutingContext routingContext) {
        String id = routingContext.request().getParam("id");
        try {
            Tweet tweet = tweetService.get(Integer.valueOf(id));
            if (tweet == null) {
